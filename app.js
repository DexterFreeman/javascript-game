...
const isCollidingWall = (shape, shapeXCoordinate, shapeYCoordinate) => {
  for (let index = 0; index < shape.length; index++) {
    ...
    if (
      (newShapeX <= 0 ||
        typeof stoppedShapeArray[newShapeX - 1][newShapeY] === "string") &&
...
const isCollidingDown = (shape, shapeXCoordinate, shapeYCoordinate) => {
  ...
  for (let index = 0; index < shapeCopy.length; index++) {
    ...
    if (currentShapeDirection === possibleDirections.down) {
      newShapeY++;
    }
...
const handleCollisionDown = (shapeXCoordinate, shapeYCoordinate, newShape) => {
  ...
  //If not, stop draw the shape, and continue playing.
    ...
    checkAndClearRows();
    createShape();
    currentShapeDirection = possibleDirections.idle;
    currentShapeX = 4;
    currentShapeY = 0;
    drawCurrentShape();
  }
...
const moveShapeDown = () => {
  currentShapeDirection = possibleDirections.down;
...
const resetGame = () => {
  ...
  gameBoardArray = [...Array(gameBoardArrayHeight)].map((e) =>
    Array(gameBoardArrayWidth).fill(0)
  );
...
const rotateShape = () => {
  ...
  if (
    isCollidingDown(newShape, currentShapeX, currentShapeY) ||
    isCollidingWall(newShape, currentShapeX, currentShapeY)
  ) {
  } else {
    deleteCurrentShape();
